...
$HistorialTransaccion = HistorialTransaccion::getInstance();

if(Vars::post(Init::ACTION))
{
	
	if(Vars::post(Init::ACTION) ==  Init::JEDIT):
		$HistorialTransaccion->Save();
	endif;	

	if(Vars::post(Init::ACTION) ==  Init::JDELETE):
		$delete = explode(",",Vars::post(Init::JID));
		foreach($delete as $id):
		$HistorialTransaccion->setDao($HistorialTransaccion->getId($id));
			$HistorialTransaccion->Delete();
...
else
{
	...
	$data = $HistorialTransaccion->Filtrar($filtro);//OBTTENER RESULTADO EN BASE A LOS FILTOS INGRESADOS
